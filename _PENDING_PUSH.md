...
**Email notification done too (commit 2c9568f, verified deployed):** this site never used Netlify's own dashboard Form notifications feature, it uses a shared function (`netlify/functions/submission-created.mjs`) with a hardcoded `NOTIFY_FORMS` allowlist. Added `britni-deep-dive` to it with its own subject/body. `rj@balaynibruno.co` now gets emailed the full answers the moment she submits. Confirmed via the Netlify Forms API that the form itself was already correctly registered (id `6ab3d75cefe59400087c97b7`, all 41 fields detected).
